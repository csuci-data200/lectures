...
Last class we learned the normal distribution:

- for random variables whose possible values range from $-\infty$ to $\infty$
- defined by two parameters: mean $\mu$ and variance $\sigma^2$
- symmetric

# Beta distribution

Today we will learn about the beta distribution

- for random variables whose possible values range from 0 to 1
- defined by two shape parameters: $\alpha$ and $\beta$ (less interpretable)
...
betas <- data.frame(
  setting = factor(rep(1:9, each = 500)), 
...
levels(betas$setting) <- paste0("Beta(",alpha,",",beta,")")
...
trend_data <- data.frame(
  ...
  mutate(Parameter = paste0("Beta(",alpha,",",beta,")")) %>% 
  mutate(setting = Parameter) %>% 
  mutate(
    means_d = dbeta(means, alpha, beta), 
    modes_d = dbeta(modes, alpha, beta)
  )

trend_data$setting <- factor(
  trend_data$setting, 
  levels = c(
    "Beta(1,5)",
    "Beta(1,2)",
...
*Do this in your lab7*

## Data

```{r}
set.seed(84735)
bechdel_sample <- sample_n(bechdel, size = 20)
```
...
```{r}
summarize_beta_binomial(alpha = 1, beta = 1, y = 9, n = 20)
```
...
library(patchwork)
optimist <- plot_beta_binomial(alpha = 14, beta = 1, y = 9, n = 20) +
  labs(title = "Optimist")

clueless <- plot_beta_binomial(alpha = 1, beta = 1, y = 9, n = 20) +
  labs(title = "Clueless")

realist <- plot_beta_binomial(alpha = 5, beta = 11, y = 9, n = 20) +
  labs(title = "Realist")

gridExtra::grid.arrange(optimist,  clueless, realist, ncol = 3)
```
...
Morteza, Nadide, and Ursula –  all share the optimistic Beta(14,1) prior for $\pi$ but each have access to different data. 

- Morteza reviews movies from 1991,
- Nadide reviews movies from 2000,
- Ursula reviews movies from 2013. 

How will the posterior distribution for each differ?
...
```{r}
summarize_beta_binomial(alpha = 14, beta = 1, y = 6, n = 13)
```
...
```{r}
summarize_beta_binomial(alpha = 14, beta = 1, y = 29, n = 63)
```
...
```{r}
summarize_beta_binomial(alpha = 14, beta = 1, y = 46, n = 99)
```
...
beta_binom_grid_plot <- function(data, likelihood = FALSE, posterior = FALSE){
  ...
  if(likelihood == TRUE){
    g <- g + 
      ...
      geom_area(
        alpha = 0.5, 
        aes(fill = "(scaled) likelihood", x = pi, y = likelihood)
      )
  }
...
make_plot_data <- function(as, bs, xs, ns, labs_prior, labs_likelihood){
  ...
  scale   <- rep(
    rep(
      c(
        integrate(bfun1, 0, 1)[[1]], 
        integrate(bfun2, 0, 1)[[1]], 
        integrate(bfun3, 0, 1)[[1]]
      ), 
      each = size
    ), 
    3
