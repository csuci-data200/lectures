...
```{r}
ave_yearly_daniel <- baby_names |> 
  filter(year >= 2000, year <= 2020) |> 
  filter(name_fixed == "DANIEL") |> 
  summarize(mean(count))
```

The average yearly number of babies born between the years 2000-2020 with the name "Daniel" was `r ave_yearly_daniel`.
...
```{r}
#| fig-cap: "Number of babies born with the name Ashley by year."
#| fig-alt: "Scatterplot of number of babies born with the name Ashley on the y-axis by year on the x-axis. Years range from approximately 1980-2020 and counts start around 2250, peak at approximately 5000 arpund 1997, and continue to reduce in popularity until 2020."
baby_names |> 
  filter(name == "ASHLEY") |> 
  ...
  labs(
    x = "Year",
    y = "Number of babies born named Ashley"
  )
```
