...
- We look for evidence against the null (p-value).

. . . 

- We make a decision (reject or fail to reject $H_0$)
  - If we find any evidence against the null then we can conclude null is false. We say we **reject the null hypothesis**.
  - If we do not find any evidence against the null then we fail to reject the null. We **fail to reject the null**.
...
We can use `pnorm()` to calculate p-value.
...
- If $H_A: \pi < ...$, you specify `lower.tail = TRUE`.
- If $H_A: \pi > ...$, you specify `lower.tail = FALSE`.
...
If the null hypothesis were true, then it would be quite unlikely (p-value = $2.6 \times 10^{-146}$) to observe a sample proportion that is at least extreme like the one we observed. 


## Decision and Conclusion

- **Decision:** It is quite unlikely that we would observe results as or more extreme as what we observed if the null were true, so we reject the $H_0$ (since p-value < 0.05).

. . .

- **Conclusion** We found evidence (p-value = $2.6 \times 10^{-146}$) that the population proportion of Americans who approve allowing DACA immigrants to become citizens is not less than 0.5.

## Pretend Decision and Conclusion

Let's pretend our p-value was actually 0.3, so we can see the other way we could conclude.

. . .

- **Decision:** It is not unlikely that we would observe results as or more extreme as what we observed if the null were true, so we fail to reject the $H_0$ (since p-value > 0.05).

. . .

- **Conclusion** We failed to find evidence (p-value = 0.3) that the population proportion of Americans who approve allowing DACA immigrants to become citizens is less than 0.5.
...
3. Calculate p-value  
4. Make a Decision(reject or fail to reject $H_0$) and a Conclusion (explain your decision to someone that does not know statistics).
